...
def build_model():
    ...
    model.add(Conv2D(filters=24,kernel_size=(f_size,f_size), padding='same', input_shape=(bands, frames, num_channels)))
    model.add(MaxPooling2D(pool_size=(4, 2)))
    model.add(Activation('relu'))
    
    model.add(Conv2D(filters=48,kernel_size=(f_size,f_size), padding='same'))
    model.add(MaxPooling2D(pool_size=(4, 2)))
    model.add(Activation('relu'))
    ...
    adam = Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0)
...
if __name__ == "__main__":
    ...
    filepath = "models/salamon-cnn-model1.h5"
    model.save(filepath)